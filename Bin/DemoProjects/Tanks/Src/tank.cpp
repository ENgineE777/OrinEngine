...
namespace Oak
{
	...
	void Tank::AddInstance(int id, Math::Vector3 pos, bool is_bot)
	{
	...
	bool Tank::Play()
	{
		...
		eastl::vector<Scene::Group*> out_groups;
		...
		for (auto group : out_groups)
		{
			...
			{
				client = (TankClient*)group->entities[0];
				break;
			}
		}
	...
	void Tank::Update(float dt)
	{
		...
		if (!bonuses_places)
		{
			eastl::vector<Scene::Group*> out_group_bot;
			...
			for (auto group : out_group)
			{
				if (group->entities.size() > 0)
				{
					Terrain* terrain = (Terrain*)group->entities[0];
					...
					while (z + square < terrain->hheight * 0.5f * terrain->scaleh)
					{
						...
						while (x + square < terrain->hwidth * 0.5f * terrain->scaleh)
						{
							...
							bonus.pos = Math::Vector3(x + square * Math::Rand(), 25.0f, z + square * Math::Rand());

							rcdesc.origin = bonus.pos;
							rcdesc.dir = Math::Vector3(0.0f, -1.0f, 0.0f);
		...
		for (auto& inst : client->instances)
		{
			...
			inst.serverState.tower_angel = Math::AdvanceAngle(inst.serverState.tower_angel, inst.clientState.needed_tower_angel, dt);
		...
		for (int i = 0; i < projectiles.size(); i++)
		{
				...
				if (proj.lifetime > 0.0f)
				{
					...
					if (GetRoot()->GetPhysScene()->RayCast(rcdesc))
					{
						...
						if (proj.special)
						{
							for (int i = 0; i < 5; i++)
							{
								proj.claster_pos[i] = proj.pos + Math::Vector3(-2.0f + 4.0f * Math::Rand(), 0.0f, -2.0f + 4.0f * Math::Rand());
			...
			if (proj.special && proj.state == 1)
			{
				for (int i = 0; i < 5; i++)
				{
					GetRoot()->GetRender()->DebugSphere(proj.claster_pos[i], COLOR_MAGNETA, r * 0.5f);
				}
			}
			else
			{
				GetRoot()->GetRender()->DebugSphere(proj.pos, COLOR_RED, r);
			}
		}
		...
		for (auto& bonus : bonuses)
		{
			...
			GetRoot()->GetRender()->DebugSphere(bonus.pos, colors[bonus.type], 0.5f);
		}
