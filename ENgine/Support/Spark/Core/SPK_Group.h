...
namespace SPK
{
	...
	*/
	...
	public :
		...
		static Ref<Group> create(int capacity = 100);
		~Group();
	...
	private :
		...
		struct WeakEmitterPair
		{
			...
			WeakEmitterPair(const Ref<Emitter>& obj, int nbBorn) :
				obj(obj.get()),
				nbBorn(nbBorn)
			{}
		};
		...
		int enabledParamIndices[5];
		int nbEnabledParameters;

		ColorInterpolatorDef colorInterpolator;
		FloatInterpolatorDef paramInterpolators[5];
		...
		Group(const Ref<System>& system = SPK_NULL_REF, int capacity = 100);
		Group(const Group& group);

		bool updateParticles(float deltaTime);
		void renderParticles();

		bool initParticle(int index, int& emitterIndex, int& nbManualBorn);
		void swapParticles(int index0, int index1);

		void recomputeEnabledParamIndices();

		template<typename T>
		void reallocateArray(T*& t, int newSize, int copySize);
	...
	inline Ref<Group> Group::create(int capacity)
	{
		return SPK_NEW(Group,SPK_NULL_REF,capacity);
	}

	template<typename T>
	void Group::reallocateArray(T*& t, int newSize, int copySize)
	{
		T* oldT = t;
	...
	inline int Group::getNbParticles() const
	{
		return particleData.nbParticles;
	}

	inline int Group::getCapacity() const
	{
	...
	inline const Ref<Emitter>& Group::getEmitter(int index) const
	{
		SPK_ASSERT(index < getNbEmitters(),"Group::getEmitter(int) - Index of emitter is out of bounds : " << index);
		return emitters[index];
	}

	inline int Group::getNbEmitters() const
	{
		return (int)emitters.size();
	}

	inline const Ref<Modifier>& Group::getModifier(int index) const
	{
		SPK_ASSERT(index < getNbModifiers(),"Group::geModifier(int) - Index of modifier is out of bounds : " << index);
		return modifiers[index].obj;
	}

	inline int Group::getNbModifiers() const
	{
