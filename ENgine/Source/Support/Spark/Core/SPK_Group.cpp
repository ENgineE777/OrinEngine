...
namespace SPK
{
	Group::Group(const Ref<System>& system, int capacity) :
	...
	{
		SPK_ASSERT(index < particleData.nbParticles,"Group::getParticle(int) - Particle index is out of bounds : " << index);
		return Particle(*this,index);
	}

	const Particle Group::getParticle(int index) const
	{
		SPK_ASSERT(index < particleData.nbParticles,"Group::getParticle(int) - Particle index is out of bounds : " << index);
	...
	bool Group::updateParticles(float deltaTime)
	{
		...
		int nbAutoBorn = 0;
		int nbManualBorn = nbBufferedParticles;
		...
		for (int i = 0; i < particleData.nbParticles; ++i)
		...
		for (int i = 0; i < nbEnabledParameters; ++i)
		{
	...
	void Group::reallocate(int capacity)
	{
		SPK_ASSERT(capacity != 0,"Group::reallocate(int) - Group capacity must not be 0");

		if (isInitialized() && (!particleData.initialized || capacity != particleData.maxParticles))
		{
			destroyAllAdditionnalData();

			int copySize = particleData.nbParticles;
	...
	void Group::removeEmitter(const Ref<Emitter>& emitter)
	{
		...
		if(it != emitters.end())
		{
			...
			for(int e = 0; e < activeEmitters.size(); e++)
	...
	bool Group::initParticle(int index, int& emitterIndex, int& nbManualBorn)
	{
	...
	void Group::swapParticles(int index0, int index1)
	{
	...
	void Group::innerImport(const IO::Descriptor& descriptor)
	{
        ...
        if ((attrib = descriptor.getAttributeWithValue("emitters")))
		{
			...
			for (int i = 0; i < tmpEmitters.size(); ++i)
        ...
        if ((attrib = descriptor.getAttributeWithValue("modifiers")))
		{
			...
			for (int i = 0; i < tmpModifiers.size(); ++i)
